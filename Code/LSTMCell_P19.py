            ...
            for t_i in counts: # time_group:
                ...
                if self.final_decay:
                    for feat_i in range(self.number_features):
                        # Calculate time delay based on the current time and the feature last seen time
                        delt_final = (curr_time - last_time[feat_i]).reshape(1).float()
                        # if the sensor is not measured even once do not decay
                        if last_time[feat_i] == 0:
                            pass
                        # if the sensor was measured at current time do not decay
                        elif delt_final == 0:
                            pass
                        else:
