...
class P12data(Dataset):
    def __init__(self, data_dir, name, device, splitfilename='phy12_split_subset1', padding=True, load_all=False, num_of_instances=200):
        self.data_dir = data_dir
        self.name = name
        self.load_all = load_all
        self.device=device
        
        self.data = np.load(self.data_dir + 'processed_data/PTdict_list.npy', allow_pickle=True)
        
        self.split_indices = np.load(self.data_dir + f'splits/{splitfilename}.npy', allow_pickle=True)
        # sample_idx are indices from the global list
        ...
        if not self.load_all:
            self.sample_idx = self.sample_idx[:num_of_instances]
        
        self.outcome_arr = np.load(self.data_dir + 'processed_data/arr_outcomes.npy', allow_pickle=True)
        
        self.labels = self.outcome_arr[:,-1][self.sample_idx]
    ...
    def __getitem__(self, idx):
        ...
        arr = self.data[idx]
        nfeatures = arr['arr'].shape[1]
        y = self.labels[idx]
        #print("y=",y)
        curr_len = arr['length']
        t = arr['time'].reshape(-1)
        pid = int(arr['id'])
        ...
        for ii, (row , tmp) in enumerate(zip(arr['arr'], t)):
            if ii < curr_len:
                #print(row, tmp)
                nonzero_idx = [i for i,r in enumerate(row) if r!= -1]
                for nidx in nonzero_idx:
                    time_global.append(tmp)
                    type_global.append(nidx)
                    z_global.append(row[nidx])
                    if len(delt_global) == 0:
                        delt_global.append(0)
                    else:
                        delt_global.append(tmp-t[0])
